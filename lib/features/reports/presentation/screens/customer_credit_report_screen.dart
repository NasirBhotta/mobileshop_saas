...
class CustomerCreditReportScreen extends ConsumerWidget {
  const CustomerCreditReportScreen({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final reportAsync = ref.watch(customerCreditReportProvider);
                                  ...
                                  children: [
                                    ...
                                    if (isWide)
                                      Row(
                                        ...
                                        children: [
                                          Expanded(
                                            child: _CreditCustomersCard(
                                              items: report.creditCustomers,
                                            ),
                                          ),
  ...
  Future<void> _exportCsv(BuildContext context, WidgetRef ref) async {
    final csv = await ref
        ...
        .buildCsv(reportType: BusinessReportType.customerCredit);
          ...
          builder: (context, constraints) {
            ...
            final left = Column(
              ...
              children: [
                ...
                Text(
                  'Rs ${summary.outstandingBalance.toStringAsFixed(0)}',
                ...
                const SizedBox(height: 6),
                Text(
                  '${summary.creditCustomers} credit customers out of ${summary.totalCustomers}',
                  style: Theme.of(
                    context,
            ...
            final right = Wrap(
              ...
              children: [
                Chip(
                  ...
                  label: Text('Customers ${summary.totalCustomers}'),
                ),
                Chip(
                  avatar: const Icon(Icons.credit_score_outlined, size: 16),
                  label: Text('Credit ${summary.creditCustomers}'),
                ),
                Chip(
                  avatar: const Icon(Icons.shopping_bag_outlined, size: 16),
                  label: Text('Top ${report.topCustomers.length} buyers'),
                ),
              ],
            );
...
class _SummaryGrid extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final averageOutstanding =
        summary.creditCustomers <= 0
            ? 0.0
            : summary.outstandingBalance / summary.creditCustomers;
    ...
    final items = [
      ...
      _KpiItem(
        title: 'Outstanding',
        value: 'Rs ${summary.outstandingBalance.toStringAsFixed(0)}',
        icon: Icons.account_balance_wallet_outlined,
      ),
      _KpiItem(
        title: 'Avg Credit',
        value: 'Rs ${averageOutstanding.toStringAsFixed(0)}',
        icon: Icons.analytics_outlined,
      ),
      _KpiItem(
        title: 'Top Buyers',
        value: report.topCustomers.length.toString(),
        icon: Icons.star_outline,
      ),
      _KpiItem(
        title: 'Credit Risk',
        value: summary.outstandingBalance > 0 ? 'Active' : 'Clear',
...
class _CreditCustomersCard extends StatelessWidget {
  final List<CreditCustomerReportItem> items;

  const _CreditCustomersCard({required this.items});

  @override
  Widget build(BuildContext context) {
    return _SectionCard(
      title: 'Outstanding Credit Customers',
      icon: Icons.credit_card_outlined,
      child:
          items.isEmpty
              ? const _EmptyText('No outstanding customer credit')
              : Column(
                children: [
                  const _TableHeader(
                    columns: ['Customer', 'Limit', 'Outstanding'],
                    flexes: [4, 2, 2],
                  ),
                  const Divider(),
                  for (final item in items.take(50))
                    _TableRowLine(
                      values: [
                        item.customerName,
                        'Rs ${item.creditLimit.toStringAsFixed(0)}',
                        'Rs ${item.outstandingBalance.toStringAsFixed(0)}',
                      ],
                      flexes: const [4, 2, 2],
                    ),
                ],
...
class _TopCustomersCard extends StatelessWidget {
  final List<TopCustomerReportItem> items;
  ...
  Widget build(BuildContext context) {
    return _SectionCard(
      ...
      child:
          items.isEmpty
              ? const _EmptyText('No customer sales data')
              : Column(
                children: [
                  const _TableHeader(
