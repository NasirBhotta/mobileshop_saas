-- Keep access-bearing subscription states complete regardless of whether they
-- are written by the admin lifecycle RPC, payment activation, or service role.
...
create or replace function public.validate_subscription_access_deadline()
returns trigger
language plpgsql
set search_path = public, pg_temp
as $function$
declare
  normalized_status text := lower(trim(new.status));
begin
  if normalized_status = 'trialing' and new.trial_ends_at is null then
    raise exception using
      errcode = '23514',
      message = 'A trialing subscription requires trial_ends_at.';
  end if;

  if normalized_status = 'grace_period' and new.grace_ends_at is null then
    raise exception using
      errcode = '23514',
      message = 'A grace-period subscription requires grace_ends_at.';
  end if;

  if normalized_status = 'active' and new.expires_at is null then
    raise exception using
      errcode = '23514',
      message = 'An active subscription requires expires_at.';
  end if;
...
drop trigger if exists tenant_subscriptions_validate_access_deadline
on public.tenant_subscriptions;

create trigger tenant_subscriptions_validate_access_deadline
before insert or update of status, trial_ends_at, grace_ends_at, expires_at
on public.tenant_subscriptions
for each row
execute function public.validate_subscription_access_deadline();

revoke all on function public.validate_subscription_access_deadline()
