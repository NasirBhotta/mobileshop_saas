...
begin
  ...
  -- Reverse only this PO's unpaid liability. Money/credit recovery is based
  -- solely on payments allocated to this PO, never on other supplier POs.
  select
    greatest(v_received-coalesce(sum(sp.amount),0),0),
    least(v_received,coalesce(sum(sp.amount),0))
  into v_payable,v_recovered
  from public.supplier_payments sp
  where sp.purchase_order_id=p_po_id;
  ...
  loop
    ...
    -- Keep the product catalog entry active at zero stock. Product deletion is
    -- an explicit inventory action, separate from returning a supplier PO.
  end loop;
