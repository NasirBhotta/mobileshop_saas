-- Commit a POS sale and all of its side effects exactly once per sale UUID.
-- PostgreSQL functions are transactional; any failure rolls back every write.
...
begin
  if v_sale_id is null or v_branch_id is null or v_sale_user_id is null then
    raise exception using errcode = '22023', message = 'Sale identity is incomplete.';
  end if;
  ...
  if v_sale_user_id <> auth.uid() or not exists (
    ...
    where u.id = auth.uid() and b.id = v_branch_id
  ) then
    raise exception using errcode = '42501', message = 'Sale branch is not available to this user.';
  end if;

  -- Serialize concurrent retries carrying the same idempotency key.
  ...
  if exists (select 1 from public.sales s where s.id = v_sale_id) then
    if not exists (
      ...
      where s.id = v_sale_id
        ...
        and s.user_id = v_sale_user_id
    ) then
      raise exception using errcode = '23505', message = 'Sale id is already used by another sale.';
    end if;
    return false;
  end if;
  ...
  select coalesce(sum((payment->>'amount')::numeric), 0),
         coalesce(sum((payment->>'amount')::numeric)
           filter (where lower(payment->>'method') = 'credit'), 0)
  into payment_total, credit_total
  from jsonb_array_elements(coalesce(p_sale->'sale_payments', '[]'::jsonb)) payment;

  if abs(payment_total - sale_total) > 0.01 then
    raise exception using errcode = '22023', message = 'Payment total does not match sale total.';
  end if;
  ...
  if credit_total > 0 and (
    v_customer_id is null or not exists (
      ...
      join public.branches sale_branch
        on sale_branch.id = v_branch_id
       and sale_branch.tenant_id::text = c.tenant_id::text
      where c.id = v_customer_id
    )
  ) then
    raise exception using errcode = '22023', message = 'A valid tenant customer is required for credit sales.';
  end if;

  -- Lock and validate every inventory row before creating the sale.
  for item in
  ...
  loop
    if item.quantity <= 0 then
      raise exception using errcode = '22023', message = 'Sale item quantity must be positive.';
    end if;

    perform 1 from public.inventory i
    where i.branch_id = v_branch_id and i.product_id = item.product_id
    for update;
    ...
    if not found or not exists (
      ...
      raise exception using errcode = '23514', message = 'Insufficient inventory for sale.';
    end if;
  end loop;
  ...
  insert into public.sales (
    ...
    sale_total, nullif(p_sale->>'notes', ''),
  ...
  select v_sale_id, value->>'method', (value->>'amount')::numeric
  from jsonb_array_elements(coalesce(p_sale->'sale_payments', '[]'::jsonb));
  ...
  loop
    ...
    where inventory.branch_id = v_branch_id
      and inventory.product_id = item.product_id;
  end loop;

  if credit_total > 0 then
    update public.customers
    set outstanding_balance = coalesce(outstanding_balance, 0) + credit_total
...
grant execute on function public.commit_pos_sale(jsonb) to authenticated, service_role;
