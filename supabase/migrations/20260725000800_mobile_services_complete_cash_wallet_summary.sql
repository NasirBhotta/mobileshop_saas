-- Keep physical cash, provider-wallet movement, and profit as separate
-- dashboard measures. Only completed (non-voided) transactions contribute.
...
begin
  ...
  select jsonb_build_object(
    'today_profit', coalesce(sum(t.profit_amount) filter (
      where t.transaction_at >= p_day_start
        and t.transaction_at < p_day_end
    ), 0),
    'total_profit', coalesce(sum(t.profit_amount), 0),
    'today_cash_received', coalesce(sum(t.customer_cash_amount) filter (
      where t.operation = 'send'
        and t.transaction_at >= p_day_start
        and t.transaction_at < p_day_end
    ), 0),
    'total_cash_received', coalesce(sum(t.customer_cash_amount) filter (
      where t.operation = 'send'
    ), 0),
    'today_cash_paid', coalesce(sum(t.customer_cash_amount) filter (
      where t.operation = 'receive'
        and t.transaction_at >= p_day_start
        and t.transaction_at < p_day_end
    ), 0),
    'total_cash_paid', coalesce(sum(t.customer_cash_amount) filter (
      where t.operation = 'receive'
    ), 0),
    'today_wallet_in', coalesce(sum(t.service_amount) filter (
      where t.operation = 'receive'
        and t.transaction_at >= p_day_start
        and t.transaction_at < p_day_end
    ), 0),
    'total_wallet_in', coalesce(sum(t.service_amount) filter (
      where t.operation = 'receive'
    ), 0),
    'today_wallet_out', coalesce(sum(t.service_amount) filter (
      where t.operation = 'send'
        and t.transaction_at >= p_day_start
        and t.transaction_at < p_day_end
    ), 0),
    'total_wallet_out', coalesce(sum(t.service_amount) filter (
      where t.operation = 'send'
    ), 0)
  )
  into v_result
